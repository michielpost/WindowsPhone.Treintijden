...
namespace ActueelNS.Views
{
  ...
  {
    WebSocket websocket = new WebSocket("ws://nsmlab.cloudapp.net/gps/treinen", cookies: new List<KeyValuePair<string, string>>() { new KeyValuePair<string, string>("session", "abcd") });
    MapLayer treinenLayer = new MapLayer();
    ...
    public MapPage()
    {
      ...
      this.Map.Layers.Add(treinenLayer);
